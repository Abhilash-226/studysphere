/**
 * EditClassroomPage
 * Form for tutors to edit an existing offline classroom listing
 */
...
import "../CreateClassroomPage/CreateClassroomPage.css"; // Reuse same styles
    ...
    try {
      ...
      if (response.success && response.classroom) {
        ...
        setFormData({
          ...
          feeStructure: {
            amount: classroom.feeStructure?.amount?.toString() || "",
            period: classroom.feeStructure?.period || "monthly",
          },
          location: {
            ...
            landmark: classroom.location?.landmark || "",
          },
          schedule: {
          ...
          batchInfo: {
            maxStudents: classroom.batchInfo?.maxStudents?.toString() || "",
            currentStudents: classroom.batchInfo?.currentStudents || 0,
            batchType: classroom.batchInfo?.batchType || "group",
          },
          contactInfo: {
            phone: classroom.contactInfo?.phone || "",
            whatsapp: classroom.contactInfo?.whatsapp || "",
            email: classroom.contactInfo?.email || "",
            preferredContact:
              classroom.contactInfo?.preferredContact || "whatsapp",
          },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSubmitError(response.message || "Failed to update classroom");
      }
    } catch (err) {
      setSubmitError("Failed to update classroom. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="create-classroom-page">
      <Container>
        ...
        <div className="page-header">
          ...
          <p className="text-muted">Update your classroom listing details</p>
        </div>
